...
#sum(train_BA_Id[-1] < train_BA_Id[-length(train_BA_Id)])
test_BA_Id <-data_train_DF %>% filter(is.na(BA)) %>% pull(IdNumber)
#sum(test_BA_Id[-1] < test_BA_Id[-length(test_BA_Id)])
length(train_BA_Id)
length(test_BA_Id)
...
```{r}
SBA_cat=function(Obs, Pred,u_ba=u_ba,weights_ba=weights_ba) {
...
#  Scnt=sum(((indicatrice_cnt-prediction_cnt)^2%*%weights_cnt))
  Sba=sum((as.matrix((indicatrice_ba-prediction_ba)^2)%*%matrix(weights_ba,ncol=1)))
...
Proba= BayesUpdate(MatInfoSachantTheta= MatInfoSachantTheta,
            VectInfo = test$classe_meanBA, MatProba= matrix(as.numeric(prior_BA),byrow=T,nrow=length(test$classe_meanBA),ncol=29))

score= list(init=
              SBA_cat(Obs=test$BA, Pred=(matrix(as.numeric(prior_BA[1:28]),byrow=T,nrow=length(test$BA),ncol=28)) ,u_ba =u_ba,weights_ba=weights_ba)
           )
score= c(score, list( avecMoyBA= SBA_cat(Obs=test$BA, Pred= Proba[,1:28] ,u_ba =u_ba,weights_ba=weights_ba)
        ))
(score)
...
We now update the probability of the BA states with neighbors located at the left
...
perf_left<- SBA_cat(Obs=test$BA, Pred= Proba[,1:28] ,u_ba =u_ba,weights_ba=weights_ba)
score<-c(score, list(left=perf_left))

#l<-ls()
#rm(list=l[!l%in% c("data_train_DF", "Proba","u_ba","u_cnt","weights_ba","weights_cnt","SBA_cat","scores","CalculProbaDiscrete")])
```

We now update the probability of the BA states with neighbors located at the right
...
test=test %>% dplyr::select(-starts_with("p")) %>% 
  left_join(rbind(ProbaOK,ProbaNope)) %>% arrange(IdNumber)
Proba= test %>% dplyr::select(starts_with("p"))
perf_right<- SBA_cat(Obs=test$BA, Pred= Proba[,1:28] ,u_ba =u_ba,weights_ba=weights_ba)
...
left_join(rbind(ProbaOK,ProbaNope)) %>% arrange(IdNumber)
Proba= test %>% dplyr::select(starts_with("p"))
perf_up<- SBA_cat(Obs=test$BA, Pred= Proba[,1:28] ,u_ba =u_ba,weights_ba=weights_ba)
...
Proba= test %>% dplyr::select(starts_with("p"))
perf_down<- SBA_cat(Obs=test$BA, Pred= Proba[,1:28] ,u_ba =u_ba,weights_ba=weights_ba)
score<-c(score, list(down_pas_pris=perf_down))
test = cbind(test %>% dplyr::select(-starts_with("p")),Proba0)
...
OK <- test %>% filter(IdNumber_previous!=IdNumber) %>% 
               filter(!is.na(theta_BA_previous))
...
test=test %>% dplyr::select(-starts_with("p")) %>% left_join(rbind(ProbaOK,ProbaNope))
Proba= test %>% dplyr::select(starts_with("p"))
perf_previous<- SBA_cat(Obs=test$BA, Pred= Proba[,1:28] ,u_ba =u_ba,weights_ba=weights_ba)
score<-c(score, list(previous_pas_pris=perf_previous))
(score)
test = cbind(test %>% dplyr::select(-starts_with("p")),Proba0)
```
...
test=test %>% dplyr::select(-starts_with("p")) %>% left_join(rbind(ProbaOK,ProbaNope))
Proba= test %>% dplyr::select(starts_with("p"))
perf_next<- SBA_cat(Obs=test$BA, Pred= Proba[,1:28] ,u_ba =u_ba,weights_ba=weights_ba)
...
# Apprentissage
d<- tada %>% arrange(theta_BA) %>% dplyr::select(theta_BA,IdNumber, starts_with("lc")) %>% na.omit()
n=d %>% group_by(theta_BA) %>% summarize(n=n()) %>% pull(n)
...
if(!file.exists("M_LC_BA.Rdata")) {debut<-Sys.time() 
 for (i in 1:dim(LC)[1]){ 
       for ( j in 1:29) { M[i,j]=multinorm(theta=j, x=LC[i,])
...
fin<-Sys.time()
(fin-debut)
 save(list="M",file="M_LC_BA.Rdata") }
load(file="M_LC_BA.Rdata")
...
perf_LC<- SBA_cat(Obs=test$BA, Pred= Proba[,1:28] ,u_ba =u_ba,weights_ba=weights_ba)
...
# Apprentissage
d<- tada %>% arrange(theta_BA) %>% dplyr::select(theta_BA,IdNumber, starts_with("clim")) %>% na.omit()
n=d %>% group_by(theta_BA) %>% summarize(n=n()) %>% pull(n)
...
if(!file.exists("M_CLIM_BA.Rdata")) {debut<-Sys.time() 
...
fin<-Sys.time()
(fin-debut)
 save(list="M",file="M_CLIM_BA.Rdata") }
load(file="M_CLIM_BA.Rdata")
...
perf_CLIM<- SBA_cat(Obs=test$BA, Pred= Proba[,1:28] ,u_ba =u_ba,weights_ba=weights_ba)
...
#                                         filter(!is.na(theta_BA)) %>% 
#                                         filter(!is.nan(theta_BA)) %>% 
#                                         rename(Theta=theta_CNT, Info= theta_BA) %>% 
#                                         dplyr::select(IdNumber,Theta,Info) ,
#                                       n_Info = 29,
#                                       n_Theta=29)
# 
...
# 
# OK <- test %>% filter(!is.na(theta_BA))%>% filter(!is.nan(theta_BA))
# 
# Nope <- test %>% filter(!(IdNumber %in% OK$IdNumber))
# 
# ProbaOK= BayesUpdate(MatInfoSachantTheta= MatInfoSachantTheta,
#             VectInfo = OK$theta_BA, 
#             MatProba = OK %>% 
#               dplyr::select(starts_with("p")))
# range(ProbaOK)
# 
...
# test=test %>% dplyr::select(-starts_with("p")) %>% left_join(rbind(ProbaOK,ProbaNope))
# Proba= test %>% dplyr::select(starts_with("p"))
# perf_CA<- SCNT_cat(Obs=test$CNT, Pred= Proba[,1:28] ,u_cnt=u_cnt,weights_ba=weights_ba)
