...
data_train_DF<- data_train_DF %>% mutate(tetha_BA=cut_modif(feux=BA,u=c(u_ba,1000000)),
...
```{r}
data_train_DF %>% pull(theta_CNT) %>% table()->tab
prior_CNT= as.numeric(cumsum(tab/sum(tab)))
...
```{r}
Reduce<- function(X=rbeta(2000,1,1),K=100-1){
  breaks = unique(quantile(X, 0:K/K))
qnorm(as.integer(cut(X, breaks = breaks, include.lowest = TRUE))/(length(breaks)+1))}
...
var(LC)
SVD=svd(LC)
plot(1:18,SVD$d/sum(SVD$d))
```

```{r}
df<-data_train_DF
df<- df %>% rename(CNTcategoriel=CNT, BAcategoriel=BA)
df<-df %>% left_join(data_train_DF)

df= df %>% mutate(CNTimoins1=ifelse(month==3, yes= NA, no= CNT[IdNumber_previous]) ) %>% 
mutate(CNTiplus1=ifelse(month==9, yes= NA, no= CNT[IdNumber_next]) ) %>% mutate(CNTimoyen=mean(c(CNTiplus1,CNTiplus1),na.rm=T)) %>% 
 ...
 mutate(XiU=ifelse(IdNumber==IdNumber_up,yes=NA, no= CNT[IdNumber_up])) %>%
  ...
  mutate(Xmoyen=mean(c(XiD,XiL,XiD,XiU),na.rm=T)) 

df %>% 
  dplyr::select(CNT,Xmoyen,CNTimoyen) %>% ggplot(aes(x=CNT,y=Xmoyen))+geom_point()

library(ggmcmc)

df  %>% pairs()

%>% relocate("CNTimoins1",.after="CNT") %>% relocate(IdNumber_previous, .after="IdNumber") %>% 
