...
apply(test %>% dplyr::select(starts_with("p")),2,mean)
...
apply(Proba,2,mean)
# Sur nos tests MoyCNT n'ameliore pas ranger, on ne prend pas en compte MoyCNT
...
apply(Proba,2,mean)
# Le score ne s'améliore pas, on ne garde pas 
...
apply(Proba,2,mean)
# le voisin du mois suivant améliore
```
...
apply(Proba,2,mean)
# Pas besoin de la correlation avec BA
test = cbind(test %>% dplyr::select(-starts_with("p")),Proba0)
apply(Proba0,2,mean)
```
