...
Consider the data $(A,B)$. Via Bayes'rule one gets
$$[\theta|A,B]=\frac{[\theta,A,B]}{[A,B]}=\frac{[A,B|\theta]\times [\theta]}{[A,B]}\\
[\theta|A,B]=\frac{[A|\theta,B]\times[B|\theta]\times [\theta]}{[A,B]}$$
Focusing on these quantities as a function of the target $\theta$, we see that:
$$[{\theta}|A,B] \propto [A|\theta,B]\times[B|\theta]\times [\theta]$$
In the challenge, $\theta$ is a categorical variable refering eiter to the count class of wildfire (ranging from $1$ to $28$) in a given pixel at a certain time, or the corresponding burnt area or both. The available information $A$ stems from many various sources:
...
The tibble *Proba* now contains a prior distribution for CNT state to start with.

Some cleaning...
```{r}
l<-ls()
rm(list=l[!l%in% c("data_train_DF", "Proba")])
```
...
We now update the probability of the CNT states with neighbors

```{r}
d <- data_train_DF %>% dplyr::select(IdNumber,IdNumber_left, theta_CNT) %>% 
  arrange(IdNumber) 
d<-d %>% mutate(theta_CNT_left=d$theta_CNT[d$IdNumber_left]) %>% filter(IdNumber !=IdNumber_left) %>% na.omit()
#table(rep(c("theta1","theta2","theta3"), each=3),c("C",rep(c("A","B","C"),3)[-1]))->Tab
#Tab
#apply(Tab,1,sum)
#Tab/apply(Tab,1,sum)
#t(Tab/apply(Tab,1,sum))
Tab<- table(d$theta_CNT,d$theta_CNT_left)
Ti<-apply(Tab,1,sum) #(theta_CNT en ligne, theta_CNT_left en colonne)
theta_left_Sachant_theta=Tab/matrix(Ti,nrow = 28,ncol=28) 
theta_left_Sachant_theta_col=t(theta_left_Sachant_theta)
##
d <- data_train_DF %>% dplyr::select(IdNumber,IdNumber_left, theta_CNT) %>% 
  arrange(IdNumber) %>% mutate(theta_CNT_left=theta_CNT[IdNumber_left]) %>% left_join(Proba)
dquireste<-d %>% filter(!is.na(theta_CNT_left)) %>% filter(!(IdNumber_left==IdNumber)) %>% arrange(theta_CNT_left)
dinvariable <- d %>% filter(is.na(theta_CNT_left) | (IdNumber_left==IdNumber)) %>% arrange(IdNumber)
dquireste %>% group_by(theta_CNT_left) %>% summarize(n=n()) %>% pull(n)->n
#mise a jour bayesienne
M=matrix(theta_left_Sachant_theta_col[1,1:28],nrow=n[1],ncol=28,byrow=T)
for (s in 2:28){M= rbind(M, matrix(theta_left_Sachant_theta_col[s,1:28],nrow=n[s],ncol=28,byrow=T))
               }
Prob=M*(dquireste %>% dplyr::select(starts_with("p")))
#renormalisation
Prob =Prob/ apply(Prob,1,sum)
#reconstruction
dquireste<- bind_cols(dquireste %>% dplyr::select(-starts_with("p")),Prob)
dout<-bind_rows(dquireste,dinvariable) %>% arrange(IdNumber)
Proba<- dout %>% dplyr::select(IdNumber, theta_CNT, starts_with("p"))
#cleaning
l<-ls()
rm(list=l[!l%in% c("data_train_DF", "Proba")])
```
...
Ti<-apply(Tab,1,sum) #(theta_CNT en ligne, theta_CNT_next en colonne)
theta_next_Sachant_theta=Tab/matrix(Ti,nrow = 28,ncol=28) (se lit en colonne )
